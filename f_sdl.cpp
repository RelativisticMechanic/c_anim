...
void SDL_Display::BlitSurface(int x, int y, SDL_Texture* tex) {
	int w, h;
	...
	SDL_RenderCopy(this->renderer, tex, &srect, &drect);
}
...
SDL_Texture* SDL_Display::CreateTexFromSurf(SDL_Surface* surf) {
	return SDL_CreateTextureFromSurface(this->renderer, surf);
}
